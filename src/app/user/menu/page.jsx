...
import { Loader, AlertCircle, MapPin, Phone, Clock, Leaf, Utensils } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-green-50 via-white to-green-100 text-gray-800">
      ...
      <main className="flex-grow flex flex-col items-center px-6 py-12 w-full">
        <h1 className="text-5xl font-extrabold text-green-700 mb-12 shadow-md p-4 rounded-lg bg-white">Today's Menu</h1>
        ...
        {loading ? (
            ...
            {fetchedMenus.map((menu, index) => (
              ...
              >
                <CardHeader className="bg-green-200 p-6 border-b border-green-300">
                  <CardTitle className="text-2xl font-semibold text-green-800">{menu.mess?.name || 'No Mess Name'}</CardTitle>
                </CardHeader>
          ...
          <p className="text-xl text-gray-600 font-medium p-6 bg-white rounded-lg shadow-md">No menus available for your location.</p>
        )}
      </main>
